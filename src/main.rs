...
async fn connect(ws: WebSocket, users: Users) {
    ...
    while let Some(result) = user_rx.next().await {
        ...
        let msg = result.expect("Failed to fetch message");
        broadcast_msg(msg, &users, my_id).await; // Pass the sender's ID
    }
...
async fn broadcast_msg(msg: Message, users: &Users, sender_id: usize) {
